...
function obwp_add_menu()
{
	...
	add_menu_page($themename, $adminmenuname, 'edit_theme_options', 'obwp-settings.php');
	add_submenu_page('obwp-settings.php', 'General Settings', 'General Settings', 'edit_theme_options', 'obwp-settings.php','obwp_general_settings');
	
	//add_submenu_page('obwp-settings.php', 'Pages Settings', 'Pages Settings', 'edit_theme_options', 'obwp-settings-pages.php', 'obwp_pages_settings');
}
	...
	switch ( $value['type'] ) {
		...
		?>
	            <select style="width:300px;" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>">
	                ...
	                <option<?php if ( get_option( $value['id'] ) == $option) { echo ' selected="selected"'; $cur = true; } elseif ($option == $value['std'] && !$cur) { echo ' selected="selected"'; } ?>><?php echo $option; ?></option>
	                <?php } ?>
	            </select>
		<?php
		option_wrapper_footer($value);
