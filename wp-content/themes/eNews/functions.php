...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('eNews',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_enews.php');
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'enews_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'enews_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'enews_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'enews_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'enews_featured', 'on' ) && 'false' == et_get_option( 'enews_duplicate', 'on' ) )
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'eNews' ),
			'secondary-menu' => __( 'Secondary Menu', 'eNews' )
		)
	);
};
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		.pics div h1 a { color: #<?php echo esc_html(get_option($shortname.'_color_featheader')); ?>; }
		.mainpost-wrap h2 a, .new-post h2 a { color:#<?php echo esc_html(get_option($shortname.'_color_recentheadings')); ?>; }
		#sidebar h3 { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_titles')); ?>; }
		#footer-widgets-inside h4 { color:#<?php echo esc_html(get_option($shortname.'_color_footer_titles')); ?>; }
		.sidebar-block a, #footer-widgets-inside div.widget a { color:#<?php echo esc_html(get_option($shortname.'_color_footer_links')); ?>; }
		div.breadcrumb { color:#<?php echo esc_html(get_option($shortname.'_color_breadcrumb')); ?>; }
