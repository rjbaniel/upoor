...
?>
    <h3>
		...
		<small id="cancel-comment-reply">
	    	...
	    	<?php if ( is_user_logged_in() ) : ?>
				...
				<a href="<?php echo wp_logout_url( get_permalink() ) ?>" title="<?php echo esc_attr( __( 'Log out of this account', 'p2' ) ); ?>">
	...
	<?php else : // current user is allowed to comment ?>
 	   ...
 	   <form id="commentform" action="<?php echo site_url( 'wp-comments-post.php' ) ?>" method="post">
		        ...
		        <table>
		        	<tr>
		        		<td>
		        			...
		        			<div class="form"><input id="author" name="author" type="text" value="<?php echo esc_attr($comment_author); ?>" /></div>
		        		</td>
		        		<td>
		        			<label for="email"><?php _e('Email <em>(required)</em>', 'p2'); ?></label>
		        			<div class="form"><input id="email" name="email" type="text" value="<?php echo esc_attr($comment_author_email); ?>"  /></div>
		        		</td>
		        		<td class="last-child">
		        			<label for="url"><?php _e('Web Site', 'p2'); ?></label>
		        			<div class="form"><input id="url" name="url" type="text" value="<?php echo esc_attr($comment_author_url); ?>"  /></div>
		        		</td>
		        	</tr>
		        </table>
	        ...
	        <div>
	            <input id="comment-submit" name="submit" type="submit" value="<?php echo esc_attr( __( 'Reply', 'p2' ) ); ?>" />
	            <?php comment_id_fields() ?>&nbsp;
	            
				<span class="progress">
	                <img src="<?php echo str_replace( WP_CONTENT_DIR, content_url(), locate_template( array( 'i/indicator.gif' ) ) ) ?>" alt="<?php echo esc_attr( __('Loading...', 'p2' ) ); ?>" title='<?php echo esc_attr( __( 'Loading...', 'p2' ) ); ?>' />
	            </span>
	        </div>
