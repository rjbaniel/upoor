...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		$themename = "LightSource";
		$shortname = "lightsource";
		$default_colorscheme = "";
		...
		load_theme_textdomain('LightSource',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_lightsource.php');
...
function et_portfoliopt_additional_styles(){ ?>
	<style type="text/css">
		...
		.et_portfolio_large { margin-left: -22px !important; }
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'LightSource' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'lightsource_homepage_featured' ),
			'cat'				=> (int) get_catId( et_get_option( 'lightsource_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'lightsource_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'lightsource_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'lightsource_featured', 'on' ) && 'false' == et_get_option( 'lightsource_use_pages', 'false' ) && 'false' == et_get_option( 'lightsource_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		.post-info, #featured .post-info { color: #<?php echo esc_html(get_option($shortname.'_postinfo_color')); ?>; }
		a:link, a:visited { color: #<?php echo esc_html(get_option($shortname.'_color_mainlink')); ?>; }
		#pages ul li a:link, #pages ul li a:visited, #pages ul li a:active { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?>; }
		.sidebar-box h2 { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_titles')); ?>; }
		.titles a:link, .titles a:visited, .titles a:active, .post-wrapper h1 a, .heading2 { color:#<?php echo esc_html(get_option($shortname.'_color_heading')); ?>; }
	</style>
