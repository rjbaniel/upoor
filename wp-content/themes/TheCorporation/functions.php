...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('TheCorporation', $template_dir . '/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_thecorporation.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'TheCorporation' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'thecorporation_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'thecorporation_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	if ( 'false' == et_get_option( 'thecorporation_blog_style', 'false' ) ) return;

	/* Set the amount of posts per page on homepage */
	$query->set( 'posts_per_page', (int) et_get_option( 'thecorporation_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'thecorporation_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'thecorporation_featured', 'on' ) && 'false' == et_get_option( 'thecorporation_use_pages', 'false' ) && 'false' == et_get_option( 'thecorporation_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		#footer h3.widgettitle { color:#<?php echo esc_html(get_option($shortname.'_footer_headings')); ?> }
