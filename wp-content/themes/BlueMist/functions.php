...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('BlueMist',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_bluemist.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'BlueMist' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> '1',
			'cat'				=> (int) get_catId( et_get_option( 'bluemist_feat_cat' ) )
		) ) );
		...
		if ( $featured_query->have_posts() ) {
			while ( $featured_query->have_posts() ) {
				...
				$et_featured_post_ids[] = (int) get_the_ID();
			}
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'bluemist_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'bluemist_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'bluemist_featured', 'on' ) && 'false' == et_get_option( 'bluemist_duplicate', 'on' ) )
