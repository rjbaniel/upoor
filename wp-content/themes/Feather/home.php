				...
				<?php query_posts('page_id=' . get_pageId(html_entity_decode(get_option('feather_home_page_'.$i)))); while (have_posts()) : the_post(); ?>
					...
					<div class="blurb<?php if ( $i == 4 ) echo ' last'; ?>">
						<?php
							...
							$et_service_link = get_post_meta(get_the_ID(),'etlink',true) ? get_post_meta(get_the_ID(),'etlink',true) : get_permalink();
						?>
						<?php if ( $thumb <> '' ) { ?>
							<div class="thumb">
		...
		<div class="home-recent">
			<div class="recent-inner clearfix">
				...
				<?php
					$args=array(
						'posts_per_page' => '3',
						'cat' => (int) get_catId(get_option('feather_home_recentblog_section')),
					);
					query_posts($args);
					$blog_current_post = 1;
				?>
				...
				<a href="<?php echo esc_url(get_category_link(get_catId(get_option('feather_home_recentblog_section')))); ?>" class="blog-more"><?php esc_html_e('More','Feather'); ?></a>
