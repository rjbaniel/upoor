...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('ColdStone',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_coldstone.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'ColdStone' ),
			'secondary-menu' => __( 'Secondary Menu', 'ColdStone' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'coldstone_featured_number' ),
			'cat'				=> (int) get_catId( et_get_option( 'coldstone_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'coldstone_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'coldstone_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'coldstone_featured', 'on' ) && 'false' == et_get_option( 'coldstone_duplicate_featured', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		.tnavinner ul li a { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?>; }
		.bnavinner ul li a { color: #<?php echo esc_html(get_option($shortname.'_color_catslink')); ?>; }
		.post-info { color: #<?php echo esc_html(get_option($shortname.'_color_postinfo')); ?>; }
		.feat_post h2 a { color:#<?php echo esc_html(get_option($shortname.'_color_featheader')); ?>; }
		.feat_post { color:#<?php echo esc_html(get_option($shortname.'_color_feattext')); ?>; }
		.side_roll h3 { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_titles')); ?>; }
		.sub_article h3 a, .subpost h3 a { color:#<?php echo esc_html(get_option($shortname.'_color_headings_title')); ?>; }
		.sub_article h3 a, .subpost h3 a { background-color:#<?php echo esc_html(get_option($shortname.'_color_headingsbg_title')); ?>; }
		.single_post h2 a, .single_post h1 a { color:#<?php echo esc_html(get_option($shortname.'_color_heading')); ?>; }
	</style>
