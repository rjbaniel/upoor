...
class K2SBM {
	...
	function install() {
		add_option('k2sbm_modules', array());
	}
...
*/
	...
	function add_menus() {
		// Add the submenus
		$page = add_theme_page(__('K2 Widgets Manager','k2_domain'), __('K2 Widgets Manager','k2_domain'), 'edit_theme_options', 'k2-widgets-manager', array('K2SBM', 'module_admin'));
	...
	function list_available_widgets() {
		...
		foreach ( $wp_registered_widgets as $widget ) {
			...
			<li id="<?php echo esc_attr( $widget_id ); ?>" class="new-widget <?php if ( $is_multi ) { echo 'multi-widget'; } else { echo 'widget'; } ?>">
...
<?
	...
	function widget_control_ob_filter( $string ) {
		...
		return trim( esc_html( strip_tags( $string ) ) );
	}
}
