	...
	<?php /* Headlines for archives */ if ((!is_single() and !is_home()) or is_paged()) { ?>
		...
		<?php // Figure out what kind of page is being shown
			if (is_category()) {
				...
				printf(__('Search Results for \'%s\'','k2_domain'), esc_html($s, 1));
	...
	<?php /* Only display asides if sidebar asides are not active */ if(!$post_asides || $k2asidescheck == '0') { ?>
		...
		<div id="post-<?php the_ID(); ?>">
			<div class="entry-head">
				<h3 class="entry-title"><a href="<?php the_permalink() ?>" rel="bookmark" title='<?php printf( __('Permanent Link to "%s"','k2_domain'), esc_html(get_the_title(),1) ); ?>'><?php the_title(); ?></a></h3>
			...
			<div class="entry-content">
				...
				<?php wp_link_pages('<p><strong>'.__('Pages:','k2_domain').'</strong> ', '</p>', 'number'); ?>
			</div> <!-- .entry-content -->
