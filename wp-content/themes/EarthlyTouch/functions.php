...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		$themename = "EarthlyTouch";
		$shortname = "earthlytouch";
		$default_colorscheme = "";
		...
		load_theme_textdomain('EarthlyTouch',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_earthlytouch.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'EarthlyTouch' ),
			'secondary-menu' => __( 'Secondary Menu', 'EarthlyTouch' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'earthlytouch_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'earthlytouch_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'earthlytouch_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'earthlytouch_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'earthlytouch_featured', 'on' ) && 'false' == et_get_option( 'earthlytouch_duplicate', 'on' ) )
