...
<?php
	...
	if (!isset($no_default_header_img) || !$no_default_header_img) {
		// if we're in a category and there's a .jpg file named after it in the headers/ folder
		if (is_category()) {
			$category = array_shift(get_the_category());
		...
		// if we didn't find one but the "random headers" option is enabled
		if (!isset($no_default_header_img) || !$no_default_header_img && $mandigo_options['header_random_image']) {
...
<?php
	...
	if (isset($mandigo_options['fully_dynamic_stylesheet']) && $mandigo_options['fully_dynamic_stylesheet']) {
?>
