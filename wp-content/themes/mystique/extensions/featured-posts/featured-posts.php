...
function mystique_filter_featured_posts(){
  global $wp_query;
  ...
  if(mystique_featured_post_target_page() && $found_posts):
    // if only 1 post is set to be displayed (no slider), remove this post from the main loop (on homepage)
    if((int)get_mystique_option('featured_count') == 1) $wp_query->query_vars['post__not_in'] = array($found_post);
    $GLOBALS['valid_featured_posts'] = $found_posts;
  endif;
}
...
function mystique_featured_post() {
  if(mystique_featured_post_target_page()):
   ...
   //echo $valid_featured_posts;

   //print_r($valid_featured_posts);
...
<div id="featured-content"<?php if($count>1): ?> class="withSlider"<?php endif; ?>>
 ...
 <div class="slide-container">
  <ul class="slides">
     <?php
      if($valid_featured_posts): // only show if the global variable is set in the function above
        $number = 1;
        foreach($valid_featured_posts as $featured_post): ?>
         <!-- slide (100%) -->
          ...
          <div class="slide-content clearfix">
            <div class="details clearfix">
             <?php
              ...
              echo '<div class="summary">'.mystique_strip_string(420, strip_tags($featured_post['post_content'])).'</div>';
       ...
       <li><h4><em><?php _e("No featured posts found.", "mystique"); ?></em></h4></li><?php
      endif; ?>
   ...
   jQuery(".featured_post a").click(function () {
    ...
    jQuery.ajax({
		...
		error: function(request){
		  alert('<?php _e("Error while featuring posts","mystique"); ?>');
  ...
  <td>
    ...
    <table>
     ...
     <tr>
      ...
      <td>
       <input type="text" size="5" name="featured_count" value="<?php echo esc_html(get_mystique_option('featured_count')); ?>" />
      </td>
     </tr>
     ...
     <tr>
      ...
      <td>
       <input type="text" size="5" name="featured_timeout" value="<?php echo esc_html(get_mystique_option('featured_timeout')); ?>" />
     ...
     <tr>
      ...
      <td>
       <input type="text" size="5" name="featured_timeframe" value="<?php echo esc_html(get_mystique_option('featured_timeframe')); ?>" />
...
add_action('pre_get_posts', 'mystique_filter_featured_posts'); // get a random featured post
