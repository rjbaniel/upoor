...
function get_twitter_data() {
  if($_GET['get_twitter_data']==1):
    ...
    $id = esc_html($_GET['widget_id']);
    $twituser = esc_html($_GET['twituser']);
    $twitcount = esc_html($_GET['twitcount']);

    // based on: http://valums.com/wordpress-latest-tweets/
    $twitdata = get_option('mystique-twitter');
    $error = false;

    if ($twitdata[$id]['last_update'] < (mktime()-60)): // update every 60 seconds
        ...
        <?php if($showarchives): ?>
         ...
         <div class="box section" id="<?php echo $id; ?>-section-archives">
          ...
          <div class="box-main">
           <div class="box-content">
            <ul class="menuList">
      	     <?php //wp_get_archives(apply_filters('widwp_get_archives_args', array('type' => 'monthly', 'show_post_count' => $postcount)));
              if($postcount):
               echo preg_replace('@\<li>\<a([^>]*)>(.*?)\<\/a>([^>]*)\((.*?)\)@i', '<li><a$1 class="fadeThis"><span class="entry">$2 <span class="details inline">($4)</span></span></a>', wp_get_archives(apply_filters('widwp_get_archives_args', array('type' => 'monthly', 'show_post_count' => $postcount, 'echo' => 0))));
              else:
               echo preg_replace('@\<li>\<a([^>]*)>(.*?)\<\/a>@i', '<li><a$1 class="fadeThis"><span class="entry">$2</span></a>', wp_get_archives(apply_filters('widwp_get_archives_args', array('type' => 'monthly', 'show_post_count' => $postcount, 'echo' => 0))));
              endif;
             ?>
            </ul>
           </div>
    ...
    function widget($args, $instance){
      ...
      <?php else:
        ...
        <form action="<?php bloginfo('wpurl') ?>/wp-login.php" method="post">
         <fieldset>
           <label for="log"><?php _e("User","mystique"); ?></label><br /><input type="text" name="log" id="log" value="<?php echo esc_html(stripslashes($user_login), 1) ?>" size="20" /><br />
