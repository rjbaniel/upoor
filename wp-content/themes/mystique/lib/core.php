...
class mystique_CategoryWalker extends Walker {
  ...
  function start_el(&$output, $category, $depth, $args) {
    ...
    if($this->type == 'checkbox'):
      ...
      $output .= '<label for="opt-category-'. $category->term_id. '"> <a href="' . get_category_link($category->term_id) . '">'.esc_attr($category->name).'</a>';
      if($count) $output .= ' <span>('.$count_text.')</span> ';
      $output .= '</label>';
    else:

    if ($category->description) $title = $category->description; else $title = $count_text;
     $output .= '<a class="fadeThis '.$active_class.'" href="'.get_category_link($category->term_id).'" title="'.$title.'"><span class="title">'.esc_attr($category->name).'</span><span class="pointer"></span></a>';
   ...
   <ul id="navigation" class="clearfix">
     ...
     <?php
      if((get_option('show_on_front')<>'page') && get_mystique_option('exclude_home')<>'1'):
       if(is_home() && !is_paged()): ?>
        <li class="active home"><a class="home active fadeThis" href="<?php echo get_option('home'); ?>" title="<?php _e('You are Home','mystique'); ?>"><span class="title"><?php _e('Home','mystique'); ?></span><span class="pointer"></span></a></li>
       <?php else: ?>
     ...
     <?php
       ...
       elseif($navtype=='links'):
        ...
        foreach ($links as $link):
         if($link->link_target) $target = ' target="'.esc_html($link->link_target).'"'; else $target = '';
         if($link->link_rel) $rel = ' rel="'.esc_html($link->link_rel).'"'; else $rel = '';
         if($link->link_description) $title = ' title="'.esc_html($link->link_description).'"'; else $title = '';
...
function mystique_highlight_search_query(){
  ...
  else $query  = esc_attr(get_search_query());
  ...
  <div id="post-<?php the_ID(); ?>" class="<?php mystique_post_class('clearfix'); ?>">
   ...
   <?php if($post_settings['post_info']): ?>
     ...
     <div class="post-info clearfix <?php if ($post_thumb) echo 'with-thumbs' ?>">
      <p class="author alignleft"><?php printf(__('Posted by %1$s in %2$s','mystique'),'<a href="'. get_author_posts_url(get_the_author_meta('ID')) .'" title="'. sprintf(__("Posts by %s","mystique"), esc_attr(get_the_author())).' ">'. get_the_author() .'</a>', $category_link);
        ?>
  ...
  if (!is_single()):
    ...
    if($max_page > 1): ?>
      ...
      if ($start_page >= 2 && $pages_to_show < $max_page):
        echo '<a href="'.esc_url(get_pagenum_link()).'" class="first" title="'.__('Go to the first page','mystique').'">'.__('&laquo; First','mystique').'</a>';
        echo '<span class="extend">...</span>';
      endif;

      if($larger_page_to_show > 0 && $larger_start_page_start > 0 && $larger_start_page_end <= $max_page)
       for($i = $larger_start_page_start; $i < $larger_start_page_end; $i+=$larger_page_multiple)
        echo '<a href="'.esc_url(get_pagenum_link($i)).'" title="'.sprintf(__("Go to page %s","mystique"),$i).'">'.$i.'</a>';

      previous_posts_link('&laquo;');

      for($i = $start_page; $i <= $end_page; $i++)
       if($i == $paged) echo '<span class="current">'.$i.'</span>';
        else echo '<a href="'.esc_url(get_pagenum_link($i)).'" title="'.sprintf(__("Go to page %s","mystique"),$i).'">'.$i.'</a>';
      ...
      if ($end_page < $max_page):
        echo '<span class="extend">...</span>';
        echo '<a href="'.esc_url(get_pagenum_link($max_page)).'" class="last" title="'.__('Go to the last page','mystique').'">'.__('Last &raquo;','mystique').'</a>';
      endif; ?>
...
function mystique_curPageURL() {
  ...
  // $pageURL .= "://";
  if ($_SERVER["SERVER_PORT"] != "80") $pageURL .= "://" . $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"] . $request; else $pageURL = /*$_SERVER["SERVER_NAME"].*/ home_url( $request, $pageURL );
