...
function mystique_widget($atts){
  ...
  foreach($atts as $att=>$val):
   if ($att!="class") $instance[esc_html($att)]=esc_html($val);
  endforeach;

  $id = $class;
  $classname = $wp_widget_factory->widgets[$class]->widget_option['classname'];
...
function mystique_register_form($atts){
  ...
  <div class="form register align<?php echo esc_html($align); ?>">
    ...
    <form method="post" id="adduser" action="http://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; ?>">
     <fieldset>
      <div class="row">
        ...
        <input class="text <?php echo (!$_POST['user-name'] || esc_html($_POST['user-name']) == $labels['user-name']) ? 'clearField' : ''; ?>" name="user-name" type="text" size="30" id="user-name" value="<?php echo $error ? esc_html($_POST['user-name'], 1) : ($jquery ? $labels['user-name'] : ''); ?>" />
      </div>

      <div class="row">
        <?php if(!$jquery): ?><label for="first-name"> <?php echo $labels['first-name']; ?> </label><?php endif; ?>
      	<input class="text <?php echo (!$_POST['first-name'] || esc_html($_POST['first-name']) == $labels['first-name']) ? 'clearField' : ''; ?>" name="first-name" type="text" size="30" id="first-name" value="<?php echo $error ? esc_html($_POST['first-name'], 1) : ($jquery ? $labels['first-name'] : ''); ?>" />
      </div>

      <div class="row">
        <?php if(!$jquery): ?><label for="last-name"> <?php echo $labels['last-name']; ?> </label><?php endif; ?>
      	<input class="text <?php echo (!$_POST['last-name'] || esc_html($_POST['last-name']) == $labels['last-name']) ? 'clearField' : ''; ?>" name="last-name" type="text" size="30" id="last-name" value="<?php echo $error ? esc_html($_POST['last-name'], 1) : ($jquery ? $labels['last-name'] : ''); ?>" />
      </div>

      <div class="row">
        <?php if(!$jquery): ?><label for="email"> <?php echo $labels['email']; ?> </label><?php endif; ?>
      	<input class="text <?php echo (!$_POST['email'] || esc_html($_POST['email']) == $labels['email']) ? 'clearField' : ''; ?>" name="email" type="text" size="30" id="email" value="<?php echo $error ? esc_html($_POST['email'], 1) : ($jquery ? $labels['email'] : ''); ?>" />
      </div>

      <div class="row">
        <?php if(!$jquery): ?><label for="url"> <?php echo $labels['url']; ?> </label><?php endif; ?>
      	<input class="text <?php echo (!$_POST['url'] || esc_html($_POST['url']) == $labels['url']) ? 'clearField' : ''; ?>" name="url" type="text" size="30" id="url" value="<?php echo $error ? esc_html($_POST['url'], 1) : ($jquery ? $labels['url'] : ''); ?>" />
      </div>
      ...
      <div class="row">
        <?php if(!$jquery): ?><label for="description"> <?php echo $labels['description']; ?> </label><?php endif; ?>
      	<textarea class="textarea <?php echo (!$_POST['description'] || esc_html($_POST['description']) == $labels['description']) ? 'clearField' : ''; ?>" name="description" id="description" rows="10" cols="50"><?php echo $error ? esc_html($_POST['description'], 1) : ($jquery ? $labels['description'] : ''); ?></textarea>
      </div>
...
function mystique_googlechart($atts){
  ...
  $string .= '&chd=t:'.$data.'';
  $string .= '&chf='.$bg.'';
  return '<img title="'.esc_html($title).'" src="http://chart.apis.google.com/chart?cht='.esc_html($charttype).''.esc_html($string).esc_html($advanced).'" alt="'.esc_html($title).'" />';
}
...
function mystique_tinyurl($atts){
  ...
  if(!$title) $title = $url;
  return '<a href="'.esc_html(mystique_getTinyUrl($url)).'" rel="'.esc_html($rel).'">'.esc_html($title).'</a>';
}
...
function mystique_advertisment($atts){
  ...
  if(!empty($ad)):
   $ad = '<div class="ad align'.esc_html($align).'">'.$ad.'</div>';
   if(!$inline) $ad = '<div class="clearfix">'.$ad.'</div>';
   return $ad;
  else:
   return '<p class="error"><strong>[ad]</strong> '.sprintf(__("Empty ad slot (#%s)!","mystique"),esc_html($code)).'</p>';
  endif;
}
...
function mystique_credit(){
  return sprintf(__('%1$s theme by %2$s | Provided by %3$s', 'mystique'), '<abbr title="'.THEME_NAME.'/'.THEME_VERSION.'">Mystique</abbr>','<a href="http://digitalnature.ro">digitalnature</a>', '<a href="http://wordpress.org/">WordPress</a>');
}
