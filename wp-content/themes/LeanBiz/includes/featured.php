<?php
	...
	if (get_option('leanbiz_use_pages') == 'false') query_posts("posts_per_page=$featured_num&cat=".get_catId($featured_cat));
	else {
		global $pages_number;
		...
		if ( is_array( et_get_option( 'leanbiz_feat_pages', '', 'page' ) ) )
			$et_featured_pages_args['post__in'] = (array) array_map( 'intval', et_get_option( 'leanbiz_feat_pages', '', 'page' ) );

		query_posts( $et_featured_pages_args );
	}

	while (have_posts()) : the_post();
		$et_leanbiz_settings = maybe_unserialize( get_post_meta(get_the_ID(),'_et_leanbiz_settings',true) );
