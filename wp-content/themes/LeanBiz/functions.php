...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('LeanBiz',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_leanbiz.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'LeanBiz' )
		)
	);
}
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			...
			'cat'				=> (int) get_catId( et_get_option( 'leanbiz_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'leanbiz_homepage_posts', '6' ) );
...
if ( function_exists( 'get_custom_header' ) ) {
	...
	add_action( 'customize_register', 'et_leanbiz_customize_register' );
	function et_leanbiz_customize_register( $wp_customize ) {
		...
		$wp_customize->add_section( 'et_google_fonts' , array(
			'title'		=> __( 'Fonts', 'LeanBiz' ),
			'priority'	=> 50,
		) );

		$wp_customize->add_setting( 'leanbiz_heading_font', array(
			'default'		=> 'none',
			'type'			=> 'option',
			'capability'	=> 'edit_theme_options'
		) );

		$wp_customize->add_control( 'leanbiz_heading_font', array(
			'label'		=> __( 'Header Font', 'LeanBiz' ),
			'section'	=> 'et_google_fonts',
			'settings'	=> 'leanbiz_heading_font',
			'type'		=> 'select',
			'choices'	=> $font_choices
		) );

		$wp_customize->add_setting( 'leanbiz_body_font', array(
			'default'		=> 'none',
			'type'			=> 'option',
			'capability'	=> 'edit_theme_options'
		) );

		$wp_customize->add_control( 'leanbiz_body_font', array(
			'label'		=> __( 'Body Font', 'LeanBiz' ),
			'section'	=> 'et_google_fonts',
			'settings'	=> 'leanbiz_body_font',
			'type'		=> 'select',
			'choices'	=> $font_choices
		) );
	}

	add_action( 'wp_head', 'et_leanbiz_add_customizer_css' );
	add_action( 'customize_controls_print_styles', 'et_leanbiz_add_customizer_css' );
	function et_leanbiz_add_customizer_css(){ ?>
		<style type="text/css">
		<?php
			global $shortname;
			...
			if ( 'none' != $et_gf_heading_font || 'none' != $et_gf_body_font ) :
				...
				if ( 'none' != $et_gf_heading_font )
					et_gf_attach_font( $et_gf_heading_font, 'h1, h2, h3, h4, h5, h6, h2.featured-title, h3#reply-title, h3#comments, .wp-pagenavi, h2.title, h1.title' );
