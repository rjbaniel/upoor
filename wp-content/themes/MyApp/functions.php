...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('MyAppTheme',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_myapptheme.php');
...
function et_portfoliopt_additional_styles(){ ?>
	<style type="text/css">
		...
		.et_pt_portfolio_item { margin-left: 46px; }
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'MyAppTheme' ),
			'footer-menu' => __( 'Footer Menu', 'MyAppTheme' ),
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'myapptheme_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'myapptheme_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'myapptheme_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'myapptheme_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'myapptheme_featured', 'on' ) && 'false' == et_get_option( 'myapptheme_use_pages', 'false' ) && 'false' == et_get_option( 'myapptheme_duplicate', 'on' ) )
