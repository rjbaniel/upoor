...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('MyProduct',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_myproduct.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'MyProduct' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'myproduct_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'myproduct_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'myproduct_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'myproduct_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'myproduct_featured', 'on' ) && 'false' == et_get_option( 'myproduct_use_pages', 'false' ) && 'false' == et_get_option( 'myproduct_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		#content-left a { color: #<?php echo esc_html(get_option($shortname.'_color_mainlink')); ?>; }
		ul.nav li a { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?>; }
		ul.nav a:hover, ul.nav > li.sf-ul > a:hover, ul.nav > li.sf-ul.sfHover > a { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink_active')); ?>; }
