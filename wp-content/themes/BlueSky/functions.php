...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('Bluesky',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_bluesky.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'Bluesky' )
		)
	);
};
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'bluesky_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'bluesky_exlcats_recent', false );
