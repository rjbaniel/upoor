	...
	<?php while (have_posts()) : the_post(); ?>
		...
		if (!empty($_POST['submit'])) {
			...
			if (empty($bx_name) || empty($bx_email) || empty($bx_subject) || empty($bx_message)) {
				...
				$error_msg.= __("<p><strong>Please fill in all required fields.</strong></p>\n", 'blix');
			...
			if (!is_valid_email($bx_email)) {
				...
				$error_msg.= __("<p><strong>Your email adress failed to validate.</strong></p>\n", 'blix');
		...
		if (!$send) { ?>
			...
			<form method="post" action="<?php echo esc_attr("http://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']); ?>" id="contactform">
				...
				<fieldset>
					...
					<p><input type="submit" name="submit" value="<?php _e("Submit", 'blix'); ?>" class="button" tabindex="6" /></p>
		...
		<?php
			...
			if (@mail(get_option('admin_email'), $email_subject, $email_text, $header)) {
				...
				echo "<h2>Hey " . $displayName . __(",</h2><p>thanks for your message! I'll get back to you as soon as possible.</p>", 'blix');
