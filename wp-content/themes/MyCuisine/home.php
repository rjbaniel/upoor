...
<?php if ( get_option('mycuisine_blog_style') == 'false' ) { ?>
	...
	<div class="container clearfix">
						...
						<?php if ( get_option('mycuisine_use_area1') != 'on' ) { ?>
							...
							<?php
								$recommendations_args=array(
									'posts_per_page'=> (int) get_option('mycuisine_numposts_recommendations')
								);

								if ( false !== get_option('mycuisine_exlcats_recommendations') )
									$recommendations_args['category__not_in'] = (array) array_map( 'intval', get_option('mycuisine_exlcats_recommendations') );
		...
		<a href="<?php echo esc_url( get_option('mycuisine_menu_page_url') ); ?>" class="browse-menu"><span><?php esc_html_e('Click Here To Browse The Menu','MyCuisine'); ?></span></a>
