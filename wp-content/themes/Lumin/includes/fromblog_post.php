...
<div class="post">
	...
	<?php } ?>
	<h4><a href="<?php the_permalink() ?>" title="<?php printf(esc_attr__ ('Permanent Link to %s', 'Lumin'), get_the_title()) ?>"><?php truncate_title(24); ?></a></h4>

	<?php if (get_option('lumin_postinfo_fromblog') ) { ?>
		<p class="meta"><?php esc_html_e('Posted ','Lumin'); ?>
		<?php if (in_array('author', get_option('lumin_postinfo_fromblog'))) { esc_html_e(' by ','Lumin'); the_author_posts_link(); };
