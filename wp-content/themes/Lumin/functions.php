...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('Lumin',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_lumin.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'Lumin' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> 3,
			'cat'				=> (int) get_catId( et_get_option( 'lumin_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'lumin_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'lumin_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'lumin_featured', 'on' ) && 'false' == et_get_option( 'lumin_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		#main-area .container #main a, #main-area .container .page-block a { color: #<?php echo esc_html(get_option($shortname.'_color_mainlink')); ?>; }
		div#header ul#top-navigation li a { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?>; }
		#sidebar h3.title { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_titles')); ?>; }
		#main-area #sidebar div.widget a { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_links')); ?>; }
		div.footer-widget h4 { color:#<?php echo esc_html(get_option($shortname.'_footer_headings')); ?> }
		div.footer-widget a { color:#<?php echo esc_html(get_option($shortname.'_color_footerlinks')); ?> }
		a.comment-reply-link { color:#<?php echo esc_html(get_option($shortname.'_color_reply')); ?>; }

		h1#post-title  { color:#<?php echo esc_html(get_option($shortname.'_color_heading')); ?>; }
	</style>
