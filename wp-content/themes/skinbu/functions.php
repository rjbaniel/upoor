...
if (!defined('TEMPLATE_DOMAIN')) define('TEMPLATE_DOMAIN','skinbu');
...
load_theme_textdomain('skinbu', TEMPLATEPATH . '/languages/');
...
<?php avatar_display_comments(get_comment_author_email(),'80',''); ?>
<?php } else { ?>
<?php if (function_exists('get_avatar')) { echo get_avatar( get_comment_author_email() , 80 ); } ?>
<?php } ?>
<br /><?php comment_author_link() ?><br /><?php comment_date('F jS, Y') ?></div>

<div id="commentext"><?php comment_text() ?>	<?php if ($comment->comment_approved == '0') : ?>

 <em><?php _e('Your comment is awaiting moderation.', 'skinbu'); ?></em>

					<?php endif; ?>

		   			<?php edit_comment_link(__('edit', 'skinbu'),'&nbsp;&nbsp;',''); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
...
function skinbu_admin_header_style() { ?>
<style type="text/css">
...
add_theme_support( 'custom-header', array('admin-head-callback' => 'skinbu_admin_header_style'));
