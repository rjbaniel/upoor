<?php class CustomLogoWidget extends WP_Widget
{
  ...
  /* Displays the Widget in the front-end */
    function widget($args, $instance){
		...
		echo $before_widget;
?>
<p><img alt="" src="<?php echo esc_attr( $logoImagePath ); ?>" /></p>
<?php echo $textInfo; ?>
...
<?php
  ...
  /*Creates the form for the widget in the back-end. */
    function form($instance){
		...
		# Logo Image
		echo '<p><label for="' . $this->get_field_id('logoImagePath') . '">' . 'Logo Image URL:' . '</label><textarea cols="20" rows="2" class="widefat" id="' . $this->get_field_id('logoImagePath') . '" name="' . $this->get_field_name('logoImagePath') . '" >'. esc_textarea($logoImagePath) .'</textarea></p>';
		# Text
