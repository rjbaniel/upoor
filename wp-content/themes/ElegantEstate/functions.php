...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('ElegantEstate',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_elegantestate.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'ElegantEstate' ),
			'secondary-menu' => __( 'Secondary Menu', 'ElegantEstate' )
		)
	);
};
...
function check_listing() {
	if (isset($_REQUEST["option-listing"])) {
		...
		wp_redirect( esc_url( $category_link ) );
	}
}
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'elegantestate_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'elegantestate_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'elegantestate_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'elegantestate_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'elegantestate_featured', 'on' ) && 'false' == et_get_option( 'elegantestate_use_pages', 'false' ) && 'false' == et_get_option( 'elegantestate_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		body { background: #<?php echo esc_html(get_option($shortname.'_color_bgcolor')); ?>; }
		...
		#sidebar h4.widgettitle { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_titles')); ?>; }
		#sidebar a { color:#<?php echo esc_html(get_option($shortname.'_sidebar_links')); ?>; }
