<?php
	...
	$et_post_thumb = sprintf( $et_thumb_size_format,
		intval( get_option( 'onthego_thumbnail_width_posts', 185 ) ),
		intval( get_option( 'onthego_thumbnail_height_posts', 185 ) )
	);
	$et_page_thumb = sprintf( $et_thumb_size_format,
		intval( get_option( 'onthego_thumbnail_width_pages', 185 ) ),
		intval( get_option( 'onthego_thumbnail_height_pages', 185 ) )
	);
	$et_usual_thumb = sprintf( $et_thumb_size_format,
		intval( get_option( 'onthego_thumbnail_width_usual', 164 ) ),
		intval( get_option( 'onthego_thumbnail_height_usual', 164 ) )
	);
	...
	$et_theme_image_sizes = array(
		...
		'307x195' 	=> 'et-featured-thumb',
		'30x30' 	=> 'et-fromblog-thumb',
	);
