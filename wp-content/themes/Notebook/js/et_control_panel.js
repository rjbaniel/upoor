jQuery(document).ready(function(){
	...
	$et_sample_color.click(function(){
		var et_option_value = jQuery(this).attr('rel'),
			et_theme_folder = notebook_cp.theme_folder;
		...
		if ( jQuery(this).hasClass('et-texture') ) {
			...
			jQuery.cookie('et_notebook_texture_url', et_texture_url);
		} else {
			jQuery('body').css( 'backgroundColor', '#' + et_option_value );
			jQuery.cookie('et_notebook_bgcolor', et_option_value);
		}
	...
	jQuery('#et-control-headerfont_bg').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_notebook_header_font_color', hex);
		}
	});
	...
	jQuery('#et_control_header_font').change(function(){
		...
		jQuery.cookie('et_notebook_header_font', et_header_font_value);
	});
	...
	jQuery('#et-control-bodyfont_bg').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_notebook_body_font_color', hex);
		}
	});
	...
	jQuery('#et_control_body_font').change(function(){
		...
		jQuery.cookie('et_notebook_body_font', et_body_font_value);
	});
	...
	$et_control_close.click(function(){
		if ( jQuery(this).hasClass('control-open') ) {
			...
			jQuery.cookie('et_notebook_control_panel_open', 0);
		} else {
			$et_control_panel.animate( { left: -169 } );
			jQuery(this).addClass('control-open');
			jQuery.cookie('et_notebook_control_panel_open', 1);
		}
		return false;
	});

	if ( jQuery.cookie('et_notebook_control_panel_open') == 1 ) {
		$et_control_panel.animate( { left: -169 } );
