...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('ePhoto',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_ephoto.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'ePhoto' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'ephoto_homepage_featured' ),
			'cat'				=> (int) get_catId( et_get_option( 'ephoto_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'ephoto_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'ephoto_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'ephoto_featured', 'on' ) && 'false' == et_get_option( 'ephoto_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		ul#animate li a { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?>; }
		.categories-inside ul li a:link, .categories-inside ul li a:visited { color: #<?php echo esc_html(get_option($shortname.'_color_catslink')); ?>; }
		.post-info, .featured-info, .bottom-span2 { color: #<?php echo esc_html(get_option($shortname.'_color_postinfo')); ?>; }
		.featured-title a { color: #<?php echo esc_html(get_option($shortname.'_color_featheader')); ?>; }
		.sidebar-box-title { color:#<?php echo esc_html(get_option($shortname.'_color_sidebar_titles')); ?>; }
		.bottom-box h3 { color:#<?php echo esc_html(get_option($shortname.'_color_footer_titles')); ?>; }
		.bottom-span a:link, .bottom-span a:visited { color:#<?php echo esc_html(get_option($shortname.'_color_footer_links')); ?>; }
		h1.post-title, h1.post-title2 { color:#<?php echo esc_html(get_option($shortname.'_color_heading')); ?>; }
	</style>
