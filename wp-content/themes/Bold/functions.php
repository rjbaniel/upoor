...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('Bold',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_bold.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'Bold' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'bold_homepage_featured' ),
			'cat'				=> (int) get_catId( et_get_option( 'bold_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'bold_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'bold_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'bold_featured', 'on' ) && 'false' == et_get_option( 'bold_duplicate', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		#pages-inside li a, .current_page_item a, .home-link a { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?> !important; }
		#pages-inside li a:hover { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink_hover')); ?>!important; }
		.pics div h1 a { color: #<?php echo esc_html(get_option($shortname.'_color_featheader')); ?> !important; }
		.post-title a, .post-title-2 a, .titles-featured a { color:#<?php echo esc_html(get_option($shortname.'_color_recentheadings')); ?> !important; }
