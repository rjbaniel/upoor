...
<?php if (function_exists('wp_ozh_wsa')) { wp_ozh_wsa("336x280-almostspring-top"); } ?>
	<?php if (have_posts()) : ?>
		...
		<h2><?php _e('Archive for ','almost-spring'); echo single_cat_title(); ?></h2>
 	  	...
 	  	<?php } elseif (is_tag()) { ?>
		<h2><?php _e('Posts Tagged ','almost-spring'); single_tag_title(); ?></h2>
		...
		<?php posts_nav_link('','', __('&laquo; Previous entries','almost-spring')); ?>
		<?php posts_nav_link('', __('Next entries &raquo;','almost-spring'),''); ?>
		     <br /><br />
	<?php else : ?>
