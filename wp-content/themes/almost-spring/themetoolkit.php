...
if (!class_exists('ThemeToolkit')) {
	class ThemeToolkit{
		...
		function ThemeToolkit($theme,$array,$file){
			...
			if ( count(@preg_grep('#^\.\./themes/[^/]+/functions.php$#', get_option('active_plugins'))) > 0 ) {
				wp_cache_flush();
		...
		function add_menu() {
			...
			add_theme_page('Configure ' . $this->infos['theme_name'], $this->infos['theme_name'], 'edit_theme_options', basename($this->file), array(&$this,'admin_menu'));
		...
		function store_options($array) {
			...
			if (update_option('theme-'.$this->infos['theme_shortname'].'-options',$array)) {
				return __("Options successfully stored");
			} else {
				return __("Could not save options !");
			}
		}
		...
		function do_firstinit() {
			...
			add_option('theme-'.$this->infos['theme_shortname'].'-options',$options);
...
//			echo '<p>Thank you for installing ' . $this->infos['theme_site'] . ', a theme for Wordpress. This theme was made by '.$this->infos['theme_author'].'. </p>';
			...
			if (!$this->infos['active']) { /* theme is not active */
				echo __('<p>(Please note that this theme is currently <strong>not activated</strong> on your site as the default theme.)</p>');
			}
			...
			if ($this->infos['debug'] and $this->option) {
				...
				echo __('<p>The array <em>$'. $this->infos['classname'] . '->option</em> is actually populated with the following keys and values :</p>
				<p><pre class="updated">');
				$count = 0;
			...
			echo __('This administration menu uses <a href="http://frenchfragfactory.net/ozh/my-projects/wordpress-theme-toolkit-admin-menu/" title="Wordpress Theme Toolkit : create a admin menu for your own theme as easily as editing 3 lines">Wordpress Theme Toolkit</a> by <a href="http://frenchfragfactory.net/ozh/" title="planetOzh">Ozh</a>, modified for <a href="http://mu.wordpress.org">MU</a> by <a href="http://jason.lah.cc/">Jason Ling</a>. And everything was made possible thanks to <a href="http://wordpress.org/" title="Best. Blogware. Ever.">Wordpress</a>.</p>
			</div>
			<div id="footer"><div style="float:right;margin-left:-180px;padding-right:5%"><p><br />');
