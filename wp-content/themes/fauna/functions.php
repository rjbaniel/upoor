...
if (!defined('TEMPLATE_DOMAIN')) define('TEMPLATE_DOMAIN', 'fauna');
////////////////////////////////////////////////////////////////////////////////
// load text domain
////////////////////////////////////////////////////////////////////////////////
...
load_theme_textdomain('fauna', TEMPLATEPATH . '/languages/');
...
if ( $background ) {
$image = " background-image: url('$handle_url/$background') !important;";
...
<?php } ?>&nbsp;&nbsp;<?php if (function_exists('comment_favicon')) { ?><a href="<?php echo($comment->comment_author_url); ?>" title="<?php _e('Visit','fauna');?> <?php echo($comment->comment_author); ?>"><?php comment_favicon($before='<img src="', $after='" alt="" class="comment-avatar" />'); ?></a><?php } ?>
				<strong><?php comment_author_link(); ?></strong>
				<?php if ( function_exists(comment_subscription_status) ) { if (comment_subscription_status()) { ?><?php _e('(subscribed to comments)','fauna') ?><?php }} ?>
				<?php _e('says:','fauna') ?>
				<?php if ($comment->comment_approved == '0') : ?>
				<small><?php _e('Your comment is awaiting moderation. This is just a spam counter-measure, and will only happen the first time you post here. Your comment will be approved as soon as possible.','fauna') ?></small>
				<?php endif; ?>
			...
			<small>
				...
				<a href="#comment-<?php echo($comment->comment_ID) ?>" title="<?php _e('Permanent link to this comment','fauna') ?>">#</a>&nbsp;&nbsp;&nbsp;<?php edit_comment_link(__("Edit This",'fauna'), ' | '); ?>&nbsp;&nbsp;&nbsp;<?php comment_reply_link(array_merge( $args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
			</small>
		</div>
...
function fauna_admin_header_style() {
?>
<style type="text/css">
#headimg {
...
add_theme_support( 'custom-header', array('admin-head-callback' => 'fauna_admin_header_style'));
