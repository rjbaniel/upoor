jQuery(document).ready(function(){
	...
	$et_sample_color.click(function(){
		...
		if ( jQuery(this).hasClass('et-texture') ) {
			...
			jQuery.cookie('et_convertible_texture_url', et_texture_url);
		} else {
			...
			jQuery.cookie('et_convertible_bgcolor', et_option_value);
		}
	...
	jQuery('#et-control-background').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_convertible_bgcolor', hex);
		}
	});
	...
	jQuery('#et-control-headerfont_bg').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_convertible_header_font_color', hex);
		}
	});
	...
	jQuery('#et_control_header_font').change(function(){
		...
		if ( !jQuery( 'link#' + et_link_tag_id ).length && ! ( et_header_font_value === 'Georgia' ) )
		...
		jQuery.cookie('et_convertible_header_font', et_header_font_value);
	});
	...
	jQuery('#et-control-bodyfont_bg').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_convertible_body_font_color', hex);
		}
	});
	...
	jQuery('#et_control_body_font').change(function(){
		...
		jQuery.cookie('et_convertible_body_font', et_body_font_value);
	});
	...
	$et_control_close.click(function(){
		if ( jQuery(this).hasClass('control-open') ) {
			...
			jQuery.cookie('et_convertible_control_panel_open', 0);
		} else {
			$et_control_panel.animate( { left: -169 } );
			jQuery(this).addClass('control-open');
			jQuery.cookie('et_convertible_control_panel_open', 1);
		}
		return false;
	});

	if ( jQuery.cookie('et_convertible_control_panel_open') == 1 ) {
		$et_control_panel.animate( { left: -169 } );
