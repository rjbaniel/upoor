...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		require_once($template_dir . '/epanel/post_thumbnails_magnificent.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'Magnificent' )
		)
	);
}
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'magnificent_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'magnificent_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'magnificent_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'magnificent_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'magnificent_featured', 'on' ) && 'false' == et_get_option( 'magnificent_use_pages', 'false' ) && 'false' == et_get_option( 'magnificent_duplicate', 'on' ) )
