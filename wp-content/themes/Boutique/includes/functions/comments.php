			   ...
			   <div id="comment-<?php comment_ID(); ?>" class="clearfix">
					...
					<div class="comment-wrap clearfix">
						<div class="comment-author-metainfo">
							...
							<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
								<?php
									...
									printf( __( '%1$s at %2$s', 'twentyten' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_html__( '(Edit)', 'Boutique' ), ' ' );
								?>
						...
						<?php if ($comment->comment_approved == '0') : ?>
							<em class="moderation"><?php esc_html_e('Your comment is awaiting moderation.','Boutique') ?></em>
							<br />
						<?php endif; ?>

						<div class="comment-content"><?php comment_text() ?></div> <!-- end comment-content-->
						<?php
							$et_comment_reply_link = get_comment_reply_link( array_merge( $args, array('reply_text' => esc_attr__('Reply','Boutique'),'depth' => $depth, 'max_depth' => $args['max_depth'])) );
