			...
			<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
				<div class="entry post clearfix">
					...
					<div class="entry-content">
						<?php if (get_option('thesource_thumbnails') == 'on') { ?>
							...
							<?php $width = (int) get_option('thesource_thumbnail_width_posts');
								  $height = (int) get_option('thesource_thumbnail_height_posts');
				...
				<?php if (get_option('thesource_468_enable') == 'on') { ?>
					<?php if(get_option('thesource_468_adsense') <> '') echo(get_option('thesource_468_adsense'));
						...
						<a href="<?php echo esc_url(get_option('thesource_468_url')); ?>"><img src="<?php echo esc_attr(get_option('thesource_468_image')); ?>" alt="468 ad" class="foursixeight" /></a>
					<?php } ?>
				<?php } ?>
