...
<?php if ( have_comments() ) : ?><?php if ( ! empty($comments_by_type['comment']) ) : ?>
	...
	<h3 class="reply"><?php comments_number(__('No Responses',TEMPLATE_DOMAIN), __('One Response',TEMPLATE_DOMAIN), __('% Responses',TEMPLATE_DOMAIN ));?> <?php _e('to',TEMPLATE_DOMAIN);?>  '<?php the_title(); ?>'</h3>
<p class="comment_meta"><?php _e('Subscribe to comments with',TEMPLATE_DOMAIN); ?> <?php post_comments_feed_link(__('<abbr title="Really Simple Syndication">RSS</abbr>',TEMPLATE_DOMAIN)); ?>
<?php if ( pings_open() ) : ?>
	<?php _e('or',TEMPLATE_DOMAIN); ?> <a href="<?php trackback_url() ?>" rel="trackback"><?php _e('TrackBack',TEMPLATE_DOMAIN);?></a> <?php _e('to',TEMPLATE_DOMAIN); ?> '<?php the_title(); ?>'.
<?php endif; ?>
</p>
...
<?php if ('open' == $post-> comment_status) : ?>
<div id="respond">
<h3><?php _e('Leave a Reply',TEMPLATE_DOMAIN);?></h3>
...
<p><input name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('Submit Comment',TEMPLATE_DOMAIN);?>" title="<?php _e('Please review your comment before submitting',TEMPLATE_DOMAIN); ?>" />
