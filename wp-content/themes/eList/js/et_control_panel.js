jQuery(document).ready(function(){
	...
	$et_sample_color.click(function(){
		var et_option_value = jQuery(this).attr('rel'),
			et_theme_folder = elist_cp.theme_folder;

		if ( jQuery(this).hasClass('et-texture') ) {
			var et_texture_url = et_theme_folder + '/images/body-' + et_option_value + '.png';
			jQuery('#main-header, body.home #featured, #main-footer').css( { 'backgroundImage': 'url(' + et_texture_url + ')', 'backgroundRepeat' : 'repeat' } );
			jQuery.cookie('et_elist_texture_url', et_texture_url);
		} else {
			jQuery('#main-header, body.home #featured, #main-footer').css( 'backgroundColor', '#' + et_option_value );
			jQuery.cookie('et_elist_bgcolor', et_option_value);
		}
	...
	jQuery('#et-control-background').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			jQuery('#main-header, body.home #featured, #main-footer').css('backgroundColor', '#' + hex);
			jQuery.cookie('et_elist_bgcolor', hex);
		}
	});

	var et_header_font_elements = 'h1,h2,h3,h4,h5,h6,ul#top-menu a',
		et_header_font_elements_color = 'h1,h2,h3,h4,h5,h6,ul#top-menu > li.current_page_item > a,ul#top-menu a:hover, ul#top-menu > li.sfHover > a',
	...
	jQuery('#et-control-headerfont_bg').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_elist_header_font_color', hex);
		}
	});
	...
	jQuery('#et_control_header_font').change(function(){
		...
		jQuery.cookie('et_elist_header_font', et_header_font_value);
	});
	...
	jQuery('#et-control-bodyfont_bg').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery.cookie('et_elist_body_font_color', hex);
		}
	});
	...
	jQuery('#et_control_body_font').change(function(){
		...
		jQuery.cookie('et_elist_body_font', et_body_font_value);
	});
	...
	$et_control_close.click(function(){
		if ( jQuery(this).hasClass('control-open') ) {
			...
			jQuery.cookie('et_elist_control_panel_open', 0);
		} else {
			$et_control_panel.animate( { left: -169 } );
			jQuery(this).addClass('control-open');
			jQuery.cookie('et_elist_control_panel_open', 1);
		}
		return false;
	});

	if ( jQuery.cookie('et_elist_control_panel_open') == 1 ) {
		$et_control_panel.animate( { left: -169 } );
