...
<div id="bg2" <?php if (is_home() && get_option('13floor_featured') == 'on') echo(' id="home"'); ?>>
	<div id="wrap">
		...
		<div id="header" class="clearfix">
			...
			<!-- LOGO -->
			<a href="<?php echo home_url( '/' ); ?>"><?php $logo = (get_option('13floor_logo') <> '') ? esc_url(get_option('13floor_logo')) : get_template_directory_uri().'/images/logo.png'; ?>
			...
			if ($primaryNav == '') { ?>
				<ul class="<?php echo esc_attr( $menuClass ); ?>">
					<?php if (get_option('13floor_home_link') == 'on') { ?>
						<li <?php if (is_front_page()) echo('class="current_page_item"') ?>><a href="<?php echo home_url( '/' ); ?>"><?php esc_html_e('Home','13floor'); ?></a></li>
					<?php }; ?>
