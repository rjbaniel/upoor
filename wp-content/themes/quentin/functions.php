...
if (!defined('TEMPLATE_DOMAIN')) define('TEMPLATE_DOMAIN','quentin');

function quentin_footer_link() { ?>
<p class="credit">
<cite>&copy; <?php echo gmdate(__('Y')); ?>  <?php bloginfo('name'); ?>&nbsp;&nbsp;&nbsp;<br />
...
Theme: <a href="http://www.pikemurdy.com/quentin" rel="designer"><em>Quentin</em></a>. <?php if(function_exists('get_current_site')) { $current_site = get_current_site();  $current_network_site = get_current_site_name(get_current_site());  ?>
...
add_theme_support( 'custom-header', array('admin-head-callback' => 'quentin_admin_header_style'));
...
function quentin_widgets_init() {
	register_sidebars(1);
	wp_register_sidebar_widget('quentin_calendar_1', __('Calendar'), 'widget_quentin_calendar', null, 'calendar');
	wp_register_sidebar_widget('quentin_search_1',__('Search'), 'widget_quentin_search', null, 'search');
	wp_unregister_widget_control('quentin_calendar_1', 'calendar');
	wp_unregister_widget_control('quentin_search_1', 'search');
}
