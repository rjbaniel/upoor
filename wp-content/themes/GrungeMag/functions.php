...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('GrungeMag',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_grungemag.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'GrungeMag' ),
			'secondary-menu' => __( 'Secondary Menu', 'GrungeMag' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'grungemag_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'grungemag_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'grungemag_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'grungemag_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'grungemag_featured', 'on' ) && 'false' == et_get_option( 'grungemag_duplicate', 'on' ) )
