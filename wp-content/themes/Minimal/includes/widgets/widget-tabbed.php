...
<div id="tabbed" class="sidebar-block">
	...
	<div id="recent-tabbed" class="widget">
		...
		<ul>
			<?php query_posts("posts_per_page=$recentPostsNumber&cat=".get_catid(get_option('minimal_blog_cat')));
	...
	<div id="random-tabbed" class="widget">
		...
		<ul>
			<?php query_posts("posts_per_page=$randomNumber&caller_get_posts=1&orderby=rand&cat=".get_catid(get_option('minimal_blog_cat')));
				if (have_posts()) : while (have_posts()) : the_post(); ?>
					<?php get_template_part('includes/fromblog_post'); ?>
				<?php endwhile; endif; wp_reset_query(); ?>
		</ul>
