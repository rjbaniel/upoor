...
<?php if ( !is_home() || is_paged() ) { // Displays a home link everywhere except the home page ?>
				<li id="home-link">
					<h3><a href="<?php bloginfo('url') ?>" title="<?php echo esc_html(get_bloginfo('name'), 1) ?>"><?php _e('&lsaquo; Home', 'veryplaintxt') ?></a></h3>
				</li>
<?php } ?>
				...
				<li id="search">
					...
					<form id="searchform" method="get" action="<?php bloginfo('url') ?>">
						<div>
							<input id="s" name="s" type="text" value="<?php echo isset($_GET['s'])?esc_html(stripslashes($_GET['s']), true):''; ?>" size="10" />
...
<?php wp_list_bookmarks('title_before=<h3>&title_after=</h3>') ?>
				...
				<li id="rss-links">
					...
					<ul>
						<li><a href="<?php bloginfo('rss2_url') ?>" title="<?php echo esc_html(get_bloginfo('name'), 1) ?> RSS 2.0 Feed" rel="alternate" type="application/rss+xml"><?php _e('All posts', 'veryplaintxt') ?></a></li>
						<li><a href="<?php bloginfo('comments_rss2_url') ?>" title="<?php echo esc_html(bloginfo('name'), 1) ?> Comments RSS 2.0 Feed" rel="alternate" type="application/rss+xml"><?php _e('All comments', 'veryplaintxt') ?></a></li>
					</ul>
				</li>
