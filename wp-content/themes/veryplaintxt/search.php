...
<?php if (have_posts()) : ?>
		...
		<h2 class="page-title"><?php _e('Search Results for:', 'veryplaintxt') ?> <?php echo esc_html(stripslashes($_GET['s']), true); ?></h2>

<?php while (have_posts()) : the_post(); ?>

			<div id="post-<?php the_ID() ?>" class="<?php veryplaintxt_post_class() ?>">
				<h3 class="entry-title"><a href="<?php the_permalink() ?>" title="<?php printf(__('Permalink to %s', 'veryplaintxt'), esc_html(get_the_title(), 1)) ?>" rel="bookmark"><?php the_title() ?></a></h3>
