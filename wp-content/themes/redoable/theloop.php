...
<?php 
		if( isset($post->rp_entry_type) && 'link' == $post->rp_entry_type && !is_single()) {
    		if ($link_counter == 0) { ?>
...
<?php 	} else {
        	/* Standard Entries */
			if (isset($link_counter) && $link_counter > 0) { $link_counter = 0; ?>
...
<?php 	} // recent links close </ul> ?>
	...
	<?php if (isset($link_counter) && $link_counter > 0) { $link_counter = 0; ?>
        </ul>
    <?php } // recent links close </ul> ?>
	<!-- end add -->
