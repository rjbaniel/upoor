...
<div id="featured">
	...
	<?php
		...
		$featured_num = (int) get_option('webly_featured_num');

		if (get_option('webly_use_pages') == 'false') query_posts("posts_per_page=$featured_num&cat=".get_catId($featured_cat));
		else {
			global $pages_number;
			...
			if ( is_array( et_get_option( 'webly_feat_pages', '', 'page' ) ) )
				$et_featured_pages_args['post__in'] = (array) array_map( 'intval', et_get_option( 'webly_feat_pages', '', 'page' ) );
