...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		<?php $commentcounter++; ?>
		<li class="<?php echo $oddcomment; /* Style differently if comment author is blog author */ if ($comment->comment_author_email == get_the_author_meta('email')) { echo ' authorcomment'; } ?>" id="comment-<?php comment_ID() ?>">
...
<?php if ($post-> comment_status == "open") {?>
	<span class="commentsfeed"><?php post_comments_feed_link(__('Comments RSS', 'ml')); ?></span>
<?php }; ?>
</p>
</div>
...
<label for="author"><small><?php _e('Name', 'ml'); ?> <?php if ($req) _e('(required)'); ?></small></label></p>

<p><input class="textbox" type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" />
<label for="email"><small><?php _e('Mail (hidden)', 'ml'); ?> <?php if ($req) _e('(required)'); ?></small></label></p>
