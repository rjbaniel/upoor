...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		load_theme_textdomain('PureType',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_puretype.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'PureType' ),
			'secondary-menu' => __( 'Secondary Menu', 'PureType' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'puretype_homepage_featured' ),
			'cat'				=> (int) get_catId( et_get_option( 'puretype_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'puretype_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'puretype_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'puretype_featured', 'on' ) )
...
function et_epanel_custom_colors_css(){
	...
	<style type="text/css">
		...
		#categories { background-color: #<?php echo esc_html(get_option($shortname.'_color_catsbgcolor')); ?>; }
		a:link, a:visited { color: #<?php echo esc_html(get_option($shortname.'_color_mainlink')); ?>; }
		#pages-inside ul li a:link, #pages-inside ul li a:visited, #pages-inside ul li a:active { color: #<?php echo esc_html(get_option($shortname.'_color_pagelink')); ?>; }
		#categories ul li a:link, #categories ul li a:visited, #categories ul li a:active { color: #<?php echo esc_html(get_option($shortname.'_color_catslink')); ?>; }
		...
		.footer-box a:link, .footer-box a:visited { color:#<?php echo esc_html(get_option($shortname.'_color_footer_links')); ?>; }
		.post-title a:link, .post-title a:hover, .post-title a:visited { color:#<?php echo esc_html(get_option($shortname.'_color_heading')); ?>; }
	</style>
