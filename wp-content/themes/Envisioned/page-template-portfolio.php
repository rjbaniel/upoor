			...
			<div class="entry post clearfix">
				...
				<div id="et_pt_portfolio_gallery" class="clearfix<?php echo $et_ptemplate_portfolio_class; ?>">
					...
					<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
						...
						$et_auto_image_detection = (get_post_meta($post->ID,'et_image_detection',true) == 1) ? false : true;
						if ( has_post_thumbnail( get_the_ID() ) && $et_ptemplate_detect_portrait ) {
							...
							if ( !$et_auto_image_detection && (get_post_meta($post->ID,'et_imagetype',true) == 'p') ) {
								$height = $et_portrait_height;
							}
							if ( $et_auto_image_detection && et_is_portrait($wordpress_thumbnail_url,'',true) ) {
								$height = $et_portrait_height;
							}
						}
						...
						if ( $thumbnail["use_timthumb"] && et_is_portrait($thumb,'',$et_auto_image_detection) && $et_ptemplate_detect_portrait ) {
							if ( !$et_auto_image_detection && (get_post_meta($post->ID,'et_imagetype',true) == 'p') ) $height = $et_portrait_height;
							else if ( $et_auto_image_detection && et_is_portrait($thumb, '', $ignore_cfields) ) $height = $et_portrait_height;
						} ?>
								...
								<div class="et_pt_portfolio_item">
									...
									<div class="et_pt_portfolio_entry<?php if ( $height == $et_portrait_height ) echo ' et_portrait_layout'; ?>">
										<div class="et_pt_portfolio_image<?php if ( '' != $et_videolink || '' != $et_custom_embed_video ) echo ' et_video'; ?>">
											...
											<a class="et_portfolio_zoom_icon fancybox" title="<?php the_title_attribute(); ?>"<?php if ( '' == $et_videolink && '' == $et_custom_embed_video ) echo ' rel="portfolio"'; ?> href="<?php if ( '' != $et_videolink || '' != $et_custom_embed_video ) echo esc_url( '#' . $et_video_id ); else echo($thumbnail['fullpath']); ?>"><?php esc_html_e('Zoom in','Envisioned'); ?></a>
											<a class="et_portfolio_more_icon" href="<?php the_permalink(); ?>"><?php esc_html_e('Read more','Envisioned'); ?></a>
