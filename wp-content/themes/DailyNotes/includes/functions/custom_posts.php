...
function et_video_meta() {
	...
	$video_width = isset($custom["video_width"][0]) ? (int) $custom["video_width"][0] : '424';
	$video_height = isset($custom["video_height"][0]) ? (int) $custom["video_height"][0] : '264';
...
add_action( 'save_post', 'et_custom_post_save_details', 10, 2 );
function et_custom_post_save_details( $post_id, $post ){
	global $pagenow;
	...
	if ( isset($_POST["thumb"]) ) update_post_meta($post_id, "thumb", esc_url_raw( $_POST["thumb"] ));
	...
	if ( isset($_POST["customlink"]) ) update_post_meta($post_id, "customlink", esc_url_raw( $_POST["customlink"] ));
	if ( isset($_POST["audio-url"]) ) update_post_meta($post_id, "audio", esc_url_raw( $_POST["audio-url"] ));
}
