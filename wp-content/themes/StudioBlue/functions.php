...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		$themename = "StudioBlue";
		$shortname = "studioblue";
		$default_colorscheme = "";
		...
		load_theme_textdomain('StudioBlue',$template_dir.'/lang');

		require_once($template_dir . '/epanel/core_functions.php');

		require_once($template_dir . '/epanel/post_thumbnails_studioblue.php');
...
function register_main_menus() {
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary Menu', 'StudioBlue' ),
			'secondary-menu' => __( 'Secondary Menu', 'StudioBlue' )
		)
	);
};
...
function et_get_featured_posts_ids(){
	if ( false === ( $et_featured_post_ids = get_transient( 'et_featured_post_ids' ) ) ) {
		$featured_query = new WP_Query( apply_filters( 'et_featured_post_args', array(
			'posts_per_page'	=> (int) et_get_option( 'studioblue_featured_num' ),
			'cat'				=> (int) get_catId( et_get_option( 'studioblue_feat_cat' ) )
		) ) );
...
function et_home_posts_query( $query = false ) {
	...
	$query->set( 'posts_per_page', (int) et_get_option( 'studioblue_homepage_posts', '6' ) );

	/* Exclude categories set in ePanel */
	$exclude_categories = et_get_option( 'studioblue_exlcats_recent', false );
	...
	if ( 'on' == et_get_option( 'studioblue_featured', 'on' ) && 'false' == et_get_option( 'studioblue_duplicate', 'on' ) )
...
global $shortname;
if (get_option($shortname.'_enable_dropdowns') <> 'false') {
